pub mod mul {
    ...
    use crypto_bigint::{BoxedUint, Uint, WideWord, Word};
    ...
    /// Uses CIOS (Coarsely Integrated Operand Scanning) method.
    #[inline(always)]
    pub fn monty_mul_dynamic(a: &BoxedUint, b: &BoxedUint, modulus: &BoxedUint) -> BoxedUint {
        let a_words = a.as_words();
        let b_words = b.as_words();
        let mod_words = modulus.as_words();
        let mod_neg_inv = compute_mod_neg_inv(mod_words[0]);
        let nlimbs = modulus.nlimbs();

        let mut result: Vec<Word> = vec![0; nlimbs];
        let carry =
            montgomery_mul_cios_dynamic(a_words, b_words, mod_words, mod_neg_inv, &mut result);

        // Conditional subtraction: subtract modulus if carry != 0 OR result >= modulus
        // First compute result - modulus
        let mut diff: Vec<Word> = vec![0; modulus.nlimbs()];
        let mut borrow: Word = 0;
        for i in 0..nlimbs {
            let (d, b1) = result[i].overflowing_sub(mod_words[i]);
            let (d, b2) = d.overflowing_sub(borrow);
    ...
    #[inline(always)]
    #[allow(clippy::arithmetic_side_effects)]
    fn montgomery_mul_cios_dynamic(
        a: &[Word],
        b: &[Word],
        modulus: &[Word],
        mod_neg_inv: Word,
