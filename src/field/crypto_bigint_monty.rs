...
mod mul {
    ...
    #[inline(always)]
    pub(super) fn monty_mul<const LIMBS: usize>(
        a: &MontyField<LIMBS>,
        b: &MontyField<LIMBS>,
    ) -> MontyField<LIMBS> {
        let a_words = a.0.as_montgomery().as_words();
        let b_words = b.0.as_montgomery().as_words();
        ...
        // Reconstruct MontyForm with same params
        let result_uint = crypto_bigint::Uint::from_words(result);
        MontyField(MontyForm::from_montgomery(result_uint, *a.0.params()))
    }
    ...
    fn montgomery_mul_cios<const LIMBS: usize>(
        ...
        for i in 0..LIMBS {
            ...
            // u = acc[0] * mod_neg_inv mod 2^64
            let u = acc[0].wrapping_mul(mod_neg_inv);

            // acc += u * modulus, then shift right by one limb
            let (_, hi) = mul_add_carry(u, modulus[0], acc[0], 0);
            carry = hi;

            for j in 1..LIMBS {
                let (lo, hi) = mul_add_carry(u, modulus[j], acc[j], carry);
                acc[j - 1] = lo;
                carry = hi;
            }
